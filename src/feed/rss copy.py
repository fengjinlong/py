...
ZHIPU_API_KEY = "06314b501e4a4135b1989d56c32a2324.495r1QkUVZQmWD0W"  # ←←← 你需要填上自己的 智谱AI key
...
OUTPUT_FILE = "crypto_news_24h_zhipu_summary.csv"
MAX_WORKERS = 5 # ★ 同样从低并发开始
...
def extract_text_from_url(url):
    try:
        ...
        if not text.strip():
             tqdm.write(f"🟡 无法提取内容 (无 <p> 标签): {url}")
             return None
        return text[:3000] 
    except Exception as e:
        tqdm.write(f"⚠️ 链接抓取/提取失败：{url} - {e}")
        return None
...
def summarize_text_zhipu(text, title):
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="glm-4-air", # ★ 使用 glm-4-air 模型（速度快，效果好）
...
def fetch_rss_news():
    ...
    print("ℹ️ AI 摘要模式 (智谱AI) 已启用。") # ★ 修改日志
